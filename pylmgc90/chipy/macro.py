...
def Finalize():
  ...
  """
  global DIMENSION, echo, wdf, dict_fid, dict_fgp, fij, f2f, f2ck, inters_list
...
def ReadIni(record=0,h5_file=''):
  ...
  if h5_file:
    ...
    if not os.path.exists( os.path.join( overall_GetWorkingDirectory(),h5_file) ):
...
def OpenDisplayFiles(restart=1, mecagp_field=None, thergp_field=None, write_f2f=False, write_ck=False):
  ...
  - thergp_field (list of strings) : the list of ther GP field to display ('grad' and/or 'flux')
  - write_f2f (bool) : if the f2f files must be written
  - write_ck  (bool) : if the central kernel files must be written
  """
  global DIMENSION, wdf, dict_fid, dict_fgp, fij, f2f, f2ck, inters_list
  ...
  if config.is_vtk_display :
    ...
    if nbm > 0 :
      ...
      dict_fid['mecafe'] = (fim , mecafe_grid, {})

      writexSxxxToVTK(os.path.join(wd,'DISPLAY'))

      if mecagp_field:
        assert mecagp_field in ['stress', 'strain'], f"Unknown mecagp_field argument, must be 'stress' or 'strain' (not '{mecagp_field}')"
        fimg = startCollection(os.path.join(wd,'DISPLAY','mecagp.pvd'),wdf)
        mecagp_pd = InitMecaGpToVTK(mecagp_field)
        dict_fgp['mecagp'] = (fimg, mecagp_pd, mecagp_field)
      ...
      if with_rigid :
        fimr = startCollection(os.path.join(wd,'DISPLAY','meca_R.pvd'),wdf)
        meca_R_pd = InitMecaRToVTK(DIMENSION)
        dict_fgp['meca_R'] = (fimr, meca_R_pd, {})
    ...
    if therMAILx_GetNbTherMAILx() > 0 :
      ...
      dict_fid['therfe'] = (fin, therfe_grid, {})

      if thergp_field:
        msg = f"Unknown thergp_field argument, must be 'grad' or 'flux' (not '{thergp_field}')"
        accept = ['gradT', 'fluxT']
        assert all( f in accept for f in list(thergp_field) ) , msg
        fitg = startCollection(os.path.join(wd,'DISPLAY','thergp.pvd'),wdf)
        thergp_pd = InitTherGpToVTK(thergp_field)
        dict_fgp['thergp'] = (fitg, thergp_pd, list(thergp_field))
    ...
    if DIMENSION==2:
      ...
      if RBDY3_GetNbRBDY3() > 0:
        ...
        if 'POLYR' in tacts_dict.keys():
            writePolyrToVTK(os.path.join(wd,'DISPLAY','polyr.vtu'), tacts_dict['POLYR'][2])
    ...
    if tact_behav_GetNbTactBehav() > 0 :
      ...
      fij = startCollection(os.path.join(wd,'DISPLAY','ptc.pvd'),wdf)
      if write_f2f:
        f2f  = startCollection(os.path.join(wd,'DISPLAY','f2f.pvd'),wdf)
      if write_ck:
        f2ck = startCollection(os.path.join(wd,'DISPLAY','ck.pvd') ,wdf)


def WriteDisplayFiles(freq=1, ref_radius=None, normal_orient=None, write_gp=False, **kw):
  ...
  - write_gp (bool)  : allow the writing of GP files (must be activated in OpenDisplayFiles to have an effect)

  Optional arguments are a tuple to add external fields to visualize in case a VTK.

  Deprecated: the arguments 'ref_radius' and 'normal_orient' are now useless and will be removed in a future release
  """
  global DIMENSION, dict_fid, dict_fgp, fij, f2f, f2ck, wdf, tacts_dict, inters_list, registers2display, inter_mapper
  ...
  if TimeEvolution_GetStep()%freq == 0 :
    ...
    # reset user fields
    inter_opt={}
    for fid in dict_fid.values():
       fid[2].clear()
    for fgp in dict_fgp.values():
      if isinstance(fgp,dict):
        fgp.clear()

    # to check for deprecated arguments:
    depre = ['ref_radius', 'normal_orient',]
    msg = lambda n: f"[WARNING:WriteDsiplayFiles] '{n}' parameter is now useless.\n{' '*28}Please remove it since this will raise an error in a later release"
    # check for deprecated arguments
    for d in depre:
      if eval(d) is not None:
        print( msg(d) )

    # sort user fields depending on file
    if len(kw) > 0:

      for k,v in list(kw.items()):
        if v[0] == 'ptc':
          inter_opt[k] = v[1]
        elif v[0] == 'meca_R' and isinstance(v,dict):
          dict_fgp[v[0]][2][k] = v[1:]
        else:
          dict_fid[v[0].lower()][2][k] = v[1:]

    # write all type of file
    for k, fid in dict_fid.items() :
      writeToVTK(os.path.join(wd,'DISPLAY',k+'_'+str(wdf)),fid[0],DIMENSION,fid[1],**fid[2])

    for k, fgp in dict_fgp.items() :
      if k in ['mecagp', 'thergp']:
        if write_gp:
          writeGpToVTK(os.path.join(wd,'DISPLAY',k+'_'+str(wdf)),fgp[0],DIMENSION,fgp[1],fgp[2])
      else:
        writePdToVTK(os.path.join(wd,'DISPLAY',k+'_'+str(wdf)),fgp[0],DIMENSION,fgp[1],**fgp[2])
    ...
    if fij:
      ...
      writeIntersToVTK( os.path.join(wd,'DISPLAY','ptc_'+str(wdf)+'.vtp'), fij,
                        inters, DIMENSION, **inter_opt, **register_opt)
      if f2f or f2ck:
        f2f_inters = PRPRx_GetF2f2Inters()
        idx_prprx = inters['inter']==PRPRx_ID
        prprx_inters = inters[idx_prprx]
        if f2f:
          writeF2fToVTK( os.path.join(wd,'DISPLAY','f2f_'+str(wdf)+'.vtp'), f2f,
                         prprx_inters, f2f_inters )
        if f2ck:
          time  = TimeEvolution_GetTime()
          fname = Path(wd)/'DISPLAY'/f"ck_{str(wdf)}.vtp" 
          polyg, ck = central_kernel.get(f2f_inters, prprx_inters)
          central_kernel.write_vtk(time, fname, f2ck, polyg, ck)
...
def repack_h5file( fname ):
    ...
    working_dir = overall_GetWorkingDirectory()
    realname = os.path.join( working_dir, fname )
    subprocess.call(["h5repack", realname, realname+'.tmp'])

    # because rename cause an error on windows if file exists
    # and that replace is only available on python 3.3 (maybe earlier)
    from sys import version_info
    if version_info >= (3,3,0):
      os.replace(realname+'.tmp', realname)
    else:
      os.remove(realname)
      os.rename(realname+'.tmp', realname)
...
def checkDirectories(checkDATBOX=True):
  ...
  """
  working_dir = overall_GetWorkingDirectory()
  if( checkDATBOX and not os.path.isdir(os.path.join(working_dir,'DATBOX')) ):
    print('DATBOX directory is not present')
    sys.exit(1)
  if( not os.path.isdir(os.path.join(working_dir,'OUTBOX')) ):
    print('Creating OUTBOX directory for you...')   
    os.mkdir(os.path.join(working_dir,'OUTBOX'))
  if( not os.path.isdir(os.path.join(working_dir,'POSTPRO')) ):
    print('Creating POSTPRO directory for you...')   
    os.mkdir(os.path.join(working_dir,'POSTPRO'))
  if( not os.path.isdir(os.path.join(working_dir,'DISPLAY')) ):
    print('Creating DISPLAY directory for you...')   
    os.mkdir(os.path.join(working_dir,'DISPLAY'))

def checkInteractiveCommand():
  """Check if some specific files exist and react depending on the result:

  - file 'stop': write every possible .OUT files and exit.
  - file 'save': write every possible .LAST files.
  ...
  stop_file = os.path.join( overall_GetWorkingDirectory(), 'stop' )
  ...
  save_file = os.path.join( overall_GetWorkingDirectory(), 'save' )
  ...
  disp_file = os.path.join( overall_GetWorkingDirectory(), 'display_now' )
  if( os.path.exists(disp_file) ):
    WriteDisplayFiles()
    os.remove(disp_file)

  flush_file = os.path.join( overall_GetWorkingDirectory(), 'flush' )
