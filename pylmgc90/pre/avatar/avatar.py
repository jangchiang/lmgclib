...
#
    ...
    def imposeDrivenDof(self,group='all',component=1, description='predefined',ct=0.,amp=0.,omega=0.,
                        phi=0.,rampi=1.,ramp=0.,evolutionFile='', dofty='temp'):
        """imposeDrivenDof(self,group='all',component=1,description='predefined',ct=0.,amp=0.,omega=0.,
        phi=0.,rampi=1.,ramp=0.,evolutionFile='',dofty='temp')
        ...
        - description : 'predefined' or 'evolution'
          ...
          * 'predefined': ct, amp, omega, phi, rampi and ramp must be defined
            so that the boundary value will be : [ ct + amp * cos(omega*t+phi) ] * sgn(rampi + ramp*t) * min(abs(rampi + ramp*t), 1),
            where t is the time
        ...
        # si la C.L. imposee est de type 'predefined'
        if description == 'predefined':
           # on identife les parties de la fonction identiquement nulles
           ct_is_zero = (ct == 0.)
           cos_is_zero = (amp == 0. or (omega == 0. and phi == 0.))
           ramp_is_zero = (rampi == 0. and ramp == 0.)
           # on identifie si la ramp est inactive
           ramp_is_inactive = (abs(rampi) == 1. and ramp == 0.)

           # on en deduit la forme de la fonction

           # si la rampe est identiquement nulle
           if ramp_is_zero:
              # la fonction est identiquement nulle
              form = "f(t) = 0"
           # sinon,
           else:
              # si la constante est identiquement nulle
              if ct_is_zero:
                 # si le cosinus est identiquement nul
                 if cos_is_zero:
                    # la fonction est identiquement nulle
                    form = "f(t) = 0"
                 # sinon,
                 else:
                    # si la rampe est inactive
                    if ramp_is_inactive:
                       # la fonction se reduit a un cosinus
                       form = "f(t) = amp*cos(omega*t + phi)"
                    # sinon,
                    else:
                       # la fonction est un cosinus multiplie par une rampe
                       form = "f(t) = amp*cos(omega*t + phi)*sign(rampi + ramp*t)*min(|rampi + ramp*t|, 1)"
              # sinon,
              else:
                 # si le cosinus est identiquement nul
                 if cos_is_zero:
                 ...
                 # sinon,
                    ...
                    # si la rampe est inactive
                    if ramp_is_inactive:
                       # la fonction se reduit a un cosinus "shifte"
                       form = "f(t) = ct + amp*cos(omega*t + phi)"
                    # sinon,
                    else:
                       # la fonction est complete
                       form = "f(t) = [ct + amp*cos(omega*t + phi)]*sign(rampi + ramp*t)*min(|rampi + ramp*t|, 1)"
           
           # on affiche un message indiquant la forme de la fonction utilise pour la condition imposee
           print("Info : the boundary condition (dof : " + dofty + ", component: " + str(component) + ") on the nodes of the group\"" + \
              group + "\" of the considered avatar is of the form: " + form)          
