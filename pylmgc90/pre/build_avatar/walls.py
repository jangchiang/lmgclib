...
def roughWall3D(center, lx, ly, r, model, material, color='WALLx'):
   ...
   # on calcule le nombre de points de ces reseau
   nb_points=nbPointsInTriangularLattice2D(nb_ele=nb_ele, nb_layer=nb_layer, orientation=orientation)

   # on calcule la taille de la boite englobante de ce reseau
   [bx, by]=sizeBoundingBoxTriangularLattice2D(nb_ele=nb_ele, nb_layer=nb_layer, l=2.*r, orientation=orientation)
   print("lx=", lx)
   print("ly=", ly)
   print("nb_ele=", nb_ele)
   ...
   for i in range(nb_points):
      ...
      body.addContactors(shape='SPHER', color=color, byrd=r, shift=[coor[2*i], coor[2*i + 1], 0.])
...
def granuloRoughWall3D(center, lx, ly, rmin, rmax, model, material, color='WALLx', seed=None):
   ...
   for i in range(nb_points):
      # on ajoute un contacteur sphere au corps, centre sur le point courant du reseau
      body.addContactors(shape='SPHER', color=color, byrd=wradii[i], shift=[coor[2*i], coor[2*i + 1], wradii[i]])
   # on calcule du volume et de l'inertie du corps
   body.computeRigidProperties()
